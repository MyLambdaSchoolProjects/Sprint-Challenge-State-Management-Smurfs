...
The context API allows communication of data outsie the parent child relationship.

- [x] In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?
Actions are the functions that retrive the neccessary data from the user. Reducers are the state management that recieves the data from the actionsfired by the user. The store is the traffic cop in the middle of the intersection directing where the traffic goes.

- [x] What is the difference between Application state and Component state? When would be a good time to use one over the other?
The difference between application and component state is global vs local. Local state exists within a single component state and can only be shared through parent childrelationships, i.e. prop drilling. Global would be using advanced state management such as Redux.

- [x] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?
It allows for asyncronous update to state instead of simple syncronous updates to state. With it action-creators can have more than one action associated with them such as loading, success or failed.

- [x] What is your favorite state management system you've learned and this sprint? Please explain why!
I have not yet formed an opinion as of yet, but for complex state management redux appears to be a better choice then the standard prop drilling.
